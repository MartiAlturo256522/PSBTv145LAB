######################################################################
#  SeedCash desktop display driver and button emulator
#
...
from seedcash.emulator.virtualGPIO import GPIO
...
class desktopDisplay(threading.Thread):
    ...
    def run(self):
        ...
        title= "SeedCash Emulator v"+EMULATOR_VERSION+ " / "+controller.VERSION;

        print("*****************************************************");
        print(title);
        print("https://github.com/enteropositivo/seedcash-emulator");
        print("*****************************************************");
        ...
        self.root.configure(bg='#484D36')
        self.root.iconphoto(False, tk.PhotoImage(file='seedcash/resources/icons/emulator_icon.png'))
        # ....
        ...
        self.joystick.configure(bg='#484D36')
        ...
        self.btn1.place(x=self.width+160, y=60) # Button position dinamically (compatibility with the merged SeedCash PR #741)
        ...
        self.btn2.place(x=self.width+160, y=116) # Button position dinamically (compatibility with the merged SeedCash PR #741)
        ...
        self.btn3.place(x=self.width+160, y=172) # Button position dinamically (compatibility with the merged SeedCash PR #741)
    ...
    def ShowImage(self,Image2,Xstart,Ystart):
        ...
        if imwidth != self.width or imheight != self.height:
            if self.display_type == DISPLAY_TYPE__ILI9341: # Re-adapt Image when ILI9341 is selected (compatibility with the merged SeedCash PR #741)
    ...
    show_image = ShowImage  # Alias to call "ShowImage" as "show_image" (compatibility with the merged SeedCash PR #741)

    def update_geometry(self): # (compatibility with the merged SeedCash PR #741)
    ...
    def invert(self, enabled: bool = True): # (compatibility with the merged SeedCash PR #741)
