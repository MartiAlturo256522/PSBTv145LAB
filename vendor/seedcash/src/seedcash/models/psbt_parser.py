    ...
    def arrange_inputs_by_type_and_category(self) -> List[Dict[str, List[TxInput]]]: 
        ...
        for inp in self.inputs:
            if inp.is_token_input:
                tok = inp.spent_output.token
                category = tok.category_id
                has_nft = tok.nft_data is not None
                has_ft = tok.ft_amount is not None and tok.ft_amount > 0
                if has_nft:
...
def parse_psbt(buf) -> Dict[str, Any]:
    ...
    # Paytaca PSBT v145 writes an extra 0x00 after input maps (psbt.js
    # InputMap.serialize). deserialize skips it. Without this skip the extra
    # separator is consumed as an empty first output map and the last real
    # output map is dropped. Do not convert the PSBT to v0; skip the quirk.
    if psbt_version == 145 and pos < len(buf) and buf[pos] == 0x00:
        pos += 1
...
class PSBTParser:
    ...
    def destination_outputs(self) -> List[TxOutput]:
        """Addressable outputs in transaction index order (skips OP_RETURN)."""
        return [out for out in self.tx.outputs if out.address]

    @property
    def destination_addresses(self) -> List[str]:
        return [out.address for out in self.destination_outputs()]
