...
class LoadingPSBTView(View):
    ...
    def run(self):
        ...
        # Route from the full transaction, not only spent-input buckets.
        # Genesis creates tokens on outputs from a BCH (no-token) parent.
        if parser.inputs[0] or parser.outputs[0]:
            return Destination(PSBTNFTView, skip_current_view=True)
        elif parser.inputs[1] or parser.outputs[1]:
...
class PSBTAddressDetailsView(View):
    ...
    def run(self):
        ...
        dests = psbt_parser.destination_outputs()
        shown = dests[self.address_num] if 0 <= self.address_num < len(dests) else None
        ...
        if self.category_num is not None:
            tok = None if shown is None else shown.token
            category: Category = get_category(tok.category_id if tok else "")
            amount = tok.ft_amount if tok else 0
            if amount >= 10e8:
                self.run_screen(
                    WarningScreen,
                    title=_("High Raw Amount"),
            ...
            selected_menu_num = self.run_screen(
                ...
                title=title,
                button_data=[ButtonOption("Next Recipient" if self.address_num < psbt_parser.num_destinations - 1 else "Next")],
                selected_color=category.icon_color,
                address=shown.address if shown else self.destination_addresses[self.address_num],
                amount=amount,
                category=category,
            )
        else:
            selected_menu_num = self.run_screen(
                ...
                button_data=[ButtonOption("Next Recipient" if self.address_num < psbt_parser.num_destinations - 1 else "Next")],
                address=shown.address if shown else self.destination_addresses[self.address_num],
                amount=shown.value_satoshis if shown else 0,
            )
